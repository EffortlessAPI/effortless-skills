...
| Datatype | Postgres | Go | Python | Airtable source (input, if Airtable-connected) |
|---|---|---|---|---|
...
`_meta._conversion_metadata` below is specific to rulebooks generated by
`airtable-to-rulebook` — it's absent (or shaped differently) on a hand-authored
or LLM-direct rulebook, which is the default today.
