...
# ---------------------------------------------------------------------
# Mode A (LOCAL DEV, the DEFAULT): the consumed transpilers
# (rulebook-to-node-postgres-api / rulebook-to-vite-admin-portal) are
# actively under development, so edit-rulebook.sh defaults LOCAL_TOOL_URLS=1.
# This runs `effortless -setToolUrl` overrides pointing at
# http://host.docker.internal:$LOCAL_API_PORT and :$LOCAL_WEB_PORT (defaults
# 30039/30040 -- the ports the two tools' own start.sh scripts bind to on the
# host), so a `dotnet run` on the host is picked up immediately on the next
# rebuild -- no publish step needed for the fast local iteration loop.
#
# Mode B (PRODUCTION): set LOCAL_TOOL_URLS=0 (or any falsy value) and normal
# effortless tool resolution kicks in, using whatever URL is pinned in
# effortless.json or resolved fresh from the registry. No other change
# needed -- this script already branches on the env var. Do NOT hard-code
...
# Default ON: both consumed transpilers are under active local development,
# so the fast-iteration path (host.docker.internal -> a `dotnet run` on the
# host) is what you want unless you explicitly opt out. Set LOCAL_TOOL_URLS=0
# once those tools are published and you want normal registry resolution.
